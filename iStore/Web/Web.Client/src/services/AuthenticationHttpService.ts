...
export interface AuthenticationHttpService {
  send<T>(url: string, methodType: MethodType, headers?: ApiHeader, data?: any): Promise<ApiResponse<T>>;
}

const baseUrl = `${process.env.REACT_APP_IDENTITY_URL}`;
...
@injectable()
export default class DefaultAuthenticationHttpService implements AuthenticationHttpService {
